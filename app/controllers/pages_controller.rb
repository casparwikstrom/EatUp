class PagesController < ApplicationController
  ...
  def index
    ...
    if params[:type_ids]
      @popups = policy_scope(Popup).joins(:types).where.not(latitude: nil, longitude: nil).where(types: { id: params[:type_ids]})
      ...
      @popups = policy_scope(Popup).joins(:types).where.not(latitude: nil, longitude: nil).where(search).order(created_at: :desc).uniq
      ...
      @popups = policy_scope(Popup).where.not(latitude: nil, longitude: nil).order(created_at: :desc)
